...
//                USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, fifoemptymsk, 0);
//                intmsk.b.nptxfempty = 1;             //GINTMSK.NPTxFEmpMsk£ºnon-periodic TxFIFO Empty Mask
    ...
    while ( ep_intr )                             // Loop through the sent interrupted OUT endpoints
    {
        if (ep_intr&0x1)
        {
            ...
            if ( doepint.b.xfercompl )//DOEPINTn.XferComppl = 1 transfer completion interrupt
            {
                ...
                USBD_DCD_INT_fops->DataOutStage(pdev , epnum); //usbd_core.cÖÐµÄUSBD_DataOutStage(USB_OTG_CORE_HANDLE *pdev , uint8_t epnum)
