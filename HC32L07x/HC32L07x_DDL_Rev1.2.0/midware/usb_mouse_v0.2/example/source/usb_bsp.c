 ...
 **
 **   - 2019-06-28  1.0   First version for USB mouse demo.
 **
 ******************************************************************************/
...
{
    stc_gpio_config_t GpioInitStruct;
    ...
    //初始化鼠标左键，初始化为输入，上升沿和下降沿中断
	GpioInitStruct.enDir = GpioDirIn;
    Gpio_Init(EVB_KEY5_PORT, EVB_KEY5_PIN, &GpioInitStruct);
    Gpio_EnableIrq(EVB_KEY5_PORT, EVB_KEY5_PIN, GpioIrqFalling);
    Gpio_EnableIrq(EVB_KEY5_PORT, EVB_KEY5_PIN, GpioIrqRising);
}
...
void USB_OTG_BSP_EnableInterrupt(void)
{
    ...
    NVIC_ClearPendingIRQ(PORTD_F_IRQn);
    NVIC_SetPriority(PORTD_F_IRQn, 3);
